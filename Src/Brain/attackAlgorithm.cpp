...
bool AttackAlgorithm::canWin(gomoku_t &gameState, RowInfo &row, std::pair<int, int> pos, int depth)
{
    ...
    case Direction::HORIZONTAL:
        ...
        {
            if (i >= sizeX || (gameState.map[pos.second][i] != TILE_STATE::EMPTY && i == pos.first + 1) || (gameState.map[pos.second][i] == TILE_STATE::PLAYER2 && i > pos.first + 1))
            {
                for (i = pos.first - 1; i > pos.first - depth; i--)
                {
    ...
    case Direction::VERTICAL:
        ...
        {
            if (j >= sizeY || (gameState.map[j][pos.first] != TILE_STATE::EMPTY && j == pos.second + 1) || (gameState.map[j][pos.first] == TILE_STATE::PLAYER2 && j > pos.second + 1))
            {
                for (j = pos.second - 1; j > pos.second - depth; j--)
                {
    ...
    case Direction::LEFT_DIAGONAL:
        ...
        {
            if (i >= sizeX || j >= sizeY || (gameState.map[j][i] != TILE_STATE::EMPTY && i == pos.first + 1 && j == pos.second + 1) || (gameState.map[j][i] == TILE_STATE::PLAYER2 && i > pos.first + 1 && j > pos.second + 1))
            {
                for (i = pos.first - 1, j = pos.second - 1; i > pos.first - depth && j > pos.second - depth; i--, j--)
                {
    ...
    case Direction::RIGHT_DIAGONAL:
        for (i = pos.first - 1, j = pos.second + 1; i > pos.first - depth && j < pos.second + depth; i--, j++)
        {
            if (i < 0 || j >= sizeY || (gameState.map[j][i] != TILE_STATE::EMPTY && i == pos.first - 1 && j == pos.second + 1) || (gameState.map[j][i] == TILE_STATE::PLAYER2 && i < pos.first - 1 && j > pos.second + 1))
            {
                for (i = pos.first + 1, j = pos.second - 1; i < pos.first + depth && j > pos.second - depth; i++, j--)
                {
    ...
    for (int i = 0; i < gameState.size.y; i++)
    {
        ...
        for (int j = 0; j < gameState.size.x; j++)
        {
            ...
            if (count > 0)
            {
                ...
                rows.emplace_back(rowInfo);
                if (rowInfo.priority || count == 4)
                {
                    return rows;
                }
                count = 0;
